...
func fillCommits(email string, path string, commits map[int]int) map[int]int {
	...
	branch, _ := repo.Branches()
	err = branch.ForEach(func(p *plumbing.Reference) error {
		//  strings.TrimSuffix(folder, "/")
		// fmt.Println(p.Name())
		return nil
	})

	commit, _ := repo.CommitObjects()
	err = commit.ForEach(func(com *object.Commit) error {
		//  strings.TrimSuffix(folder, "/")

		fmt.Println(com.Author, com.Committer, com.Message, com.Parents())
		files, _ := com.Files()
		err := files.ForEach(func(files *object.File) error {
			// fmt.Println("files ", files.Name, files.Size)
			return nil
		})
		fmt.Println("err ", err)
		return nil
	})
	tree, _ := repo.TreeObjects()
	err = tree.ForEach(func(obTree *object.Tree) error {
		// fmt.Printf("obTree: %v\n", obTree)
		return nil
	})
